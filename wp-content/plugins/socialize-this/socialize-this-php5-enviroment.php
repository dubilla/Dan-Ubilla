...
class socialize_this {
	...
	public function extra_plugin_links($links, $file) {
		if($file == ST_FILE){
			$links[] = '<a href="options-general.php?page=socialize-this/socialize-this.php">' . __('Settings', 'st_plugin') . '</a>';
			$links[] = '<a href="options-general.php?page=socialize-this/socialize-this.php&module=social_widgets">' . __('Social Widgets', 'st_plugin') . '</a>';
			$links[] = '<a href="options-general.php?page=socialize-this/socialize-this.php&module=templates">' . __('Templates', 'st_plugin') . '</a>';
		}
		return $links;
	}
	...
	public function admin_menu(){
		...
		add_action( 'admin_head-'. $plugin_page, array($this, 'st_admin_jquery_css'), 10, ST_FILE, 'socialize-this');
	}
	public function st_admin_jquery_css(){
		echo '<link rel="stylesheet" type="text/css" href="'.$this->st_url.'/wp-admin.css'.'" />';
		echo '<script type="text/javascript" src="'.$this->st_url.'/jquery-1.3.2.min.js"></script>';
		if($_GET['module'] == 'social_widgets'){
			echo '<script type="text/javascript" src="'.$this->st_url.'/jquery-ui-1.7.2.custom.min.js"></script>';?>
<script type="text/javascript">
//<![CDATA[ 
function click_unclick(name){
	var widget_image = document.getElementById('widget_image_'+name);
	var widget_tickbox = document.getElementById('widget_'+name);
...
//<![CDATA[ 
$(document).ready(function(){ 
	$("[name=st_url_shortening_service]").change(function(){
		if($(this).val() == 'bitly'){
			$('#bitly_usrpss').fadeIn('slow');	
		}else if($(this).val() != 'bitly'){
			$('#bitly_usrpss').fadeOut('slow');	
		}
	});
	$("[name=st_noti_twitter]").change(function(){
		if($(this+':checked').length != 2){
			$('#twitusrpss,#twitapi').fadeOut();
		}else{
			$('#twitusrpss,#twitapi').fadeIn('slow');		
		}
	});
...
<div id="st_top_navagation"><a href="?page=socialize-this/socialize-this.php"><?php _e('Overview', 'st_plugin'); ?></a> | <a href="?page=socialize-this/socialize-this.php&module=social_widgets"><?php _e('Social Widgets', 'st_plugin'); ?></a> | <a href="?page=socialize-this/socialize-this.php&module=templates"><?php _e('Templates', 'st_plugin'); ?></a> | <a href="?page=socialize-this/socialize-this.php&module=advanced_functions"><?php _e('Advanced Functions/Settings', 'st_plugin'); ?></a></div>
<?php
	}
...
';
	...
	private function st_adm_general(){
		...
		if($_POST['Submit'] === 'Save Settings'){
			...
			if($_POST['st_noti_twitter'] == 'yes'){
				if($this->verify_twitter_credentials($_POST['st_noti_twitter_user'], stripslashes($_POST['st_noti_twitter_pass'])) == TRUE){
					...
					$note .= '<p>'.__('Twitter Authentication Failed. Check your credentials').'</p>';
				}
			}else{
				update_option('st_noti_twitter', 'no');	
				// wipe the data
				update_option('st_noti_twitter_user', NULL);
				update_option('st_noti_twitter_pass', NULL);
			}
			
			update_option('st_url_shortening_service', $_POST['st_url_shortening_service']);
			if($_POST['st_url_shortening_service'] == 'bitly'){
				if($this->verify_bitly($_POST['st_bitly_user'], $_POST['st_bitly_apikey']) == true){
...
<em>
      ...
      <tr>
        <td width="40%"><?php _e('Amount of widgets until a', 'st_plugin'); ?>
          &lt;br /&gt;<br />
          <em>
          <?php _e('For changing values for each line, sperate each value with a comma. -1 means unlimited.', 'st_plugin'); ?>
          </em></td>
        <td width="60%"><label>
      ...
      <tr>
        ...
        <td width="60%"><label>
            ...
            <option value="bitly"<?php if(get_option('st_url_shortening_service') == 'bitly'){echo ' selected="selected"';}?>>bit.ly</option>
            </select>
          </label></td>
      </tr>
      <tr <?php if(get_option('st_url_shortening_service') != 'bitly'){echo ' class="hidden"';}?> id="bitly_usrpss">
        <td width="40%"><?php _e('Bit.ly Account Information', 'st_plugin'); ?>
          <br />
          <em>
          <?php _e('Your Bit.Ly Username and API key. This also requires cURL.', 'st_plugin'); ?>
          </em></td>
        <td width="60%">
          <label>
      ...
      <tr>
        ...
        <td width="60%"><label>
            <?php _e('Enable:', 'st_plugin'); ?> <input type="checkbox" name="st_noti_twitter" id="checkbox" value="yes" <?php if(get_option('st_noti_twitter') === 'yes'){?> checked="checked" <?php } ?>>
          </label>
          <div id="twitusrpss" <?php if(get_option('st_noti_twitter') != 'yes'){?> class="hidden" <?php } ?>>
          <label>
            <?php _e('Username:', 'st_plugin'); ?> 
      ...
      <tr  id="twitapi" <?php if(get_option('st_noti_twitter') != 'yes'){?> class="hidden" <?php } ?>>
        <td width="40%"><?php _e('Twitter\'s @Anywhere API Key', 'st_plugin'); ?>
          <br />
          <em>
          <?php _e('This will enable Widgets to use your @Anywhere key (%anywhere%)', 'st_plugin'); ?>
          </em></td>
        <td width="60%"><label>
            Api Key: <input type="text" name="st_twitter_apikey" value="<?php echo get_option('st_twitter_apikey'); ?>" />
          </label>
         </td>
      </tr>
    </table>
  ...
  <form action="" method="post" enctype="application/x-www-form-urlencoded">
    <h4>
      <?php _e('Social Widgets (Click to enable/disable. Drag and drop to change order)', 'st_plugin'); ?>
    </h4>
    <div id="widgetOrder" align="center">
      <ul>
        <?php 
	  //asort($this->st_imgs);
  ...
  <h3>
    <?php _e('Install a Widget Set', 'st_plugin'); ?>
  </h3>
  <form action="" method="post" enctype="application/x-www-form-urlencoded">
  ...
  <h3>
    <?php _e('Installed Widget Sets', 'st_plugin'); ?>
  </h3>
  <form action="" method="post" enctype="application/x-www-form-urlencoded">
    <div class="widget_boxes">
      <?php
  ...
  <p>
    ...
    : <a href="http://www.fullondesign.co.uk/" target="_blank">Mike Rogers (Full On Design)</a> | <a href="http://twitter.com/Rogem002" target="_blank">Rogem002 (Twitter)</a> (
    <?php _e('For Support Visit the', 'st_plugin');?>
    <a href="http://www.fullondesign.co.uk/socialize-this" target="_blank">Socialize This</a> <?php _e('homepage', 'st_plugin');?>)<br />
	...
	private function st_adm_advanced_functions(){
		if(isset($_POST['save_advanced_settings'])){
			if($_POST['st_enable_api'] == 'yes'){update_option('st_enable_api', TRUE);} else {update_option('st_enable_api', FALSE);}
		...
		?>
		<h3>
    <?php _e('Advanced Settings', 'st_plugin'); ?>
  </h3>
  <form action="" method="post" enctype="application/x-www-form-urlencoded">
    <table width="90%" border="0" class="widefat post fixed">
    <thead>
    ...
    <tr>
      ...
      <tr>
        <td width="40%"><?php _e('Show template delete buttons', 'st_plugin'); ?></td>
        <td width="60%"><label><?php _e('Enable:', 'st_plugin'); ?> 
      ...
      <tr>
        <td width="40%"><?php _e('Show template keys', 'st_plugin'); ?></td>
        <td width="60%"><label><?php _e('Enable:', 'st_plugin'); ?> 
  ...
  <?php
	...
	private function shorten_url($url){
		...
		elseif(get_option('st_url_shortening_service') == 'bitly'){return $this->get_link('http://api.bit.ly/v3/shorten?format=txt&login='.get_option('st_bitly_user').'&apikey='.get_option('st_bitly_apikey').'&longUrl='.urlencode($url));}
		else{return $this->get_link('http://api.tr.im/v1/trim_simple?url='.urlencode($url));}
		return $url;
	}
	
	private function verify_bitly($username, $pass){
		if(function_exists('curl_init')){
			...
			curl_setopt($twitter['ch'], CURLOPT_POST, null);
	
			// the URL it's going to AKA the twitter status updater.
			curl_setopt($twitter['ch'], CURLOPT_URL, 'http://api.bit.ly/v3/validate?format=txt&x_login=notbilytapi&x_apiKey=not_apikey&login='.$username.'&apikey='.$pass);
	...
	public function show_social_widgets($icons=NULL, $custom = NULL, $permalink=NULL, $title=NULL, $excerpt=NULL){
		global $post;
		$output = '';
		if (is_object($post) && $custom == NULL){
			$permalink = get_permalink($post->ID);
			asort($this->st_imgs);
			$count = 0;
		...
		if($custom == TRUE && $permalink != NULL && $title!=NULL){
			asort($this->st_imgs);
			$count = 0;
			$st_max_widgets_until_br_count = 0;
			...
			foreach($widget_order as $key){ // Loop the widgets
				...
				if($icons == NULL || !is_array($icons)){
					if(get_option('st_'.$key) === 'use'){ // Check if the admin wants the widget shown
						$count++;
						$output .= $this->render_social_links($permalink, $image, $title, $excerpt, 0); // Show the widget
	...
	private function render_social_links($permalink, $image, $title=NULL, $descirption=NULL, $post_id=NULL){
		...
		if(get_option('st_twitter_apikey') == true){
		...
		// Edit by Oliver Bockelmann http://www.fob-marketing.de/ - Fixes when extra blank data is on the end of the url.
        $vars['%short_url%'] = trim($vars['%short_url%']);
        $vars['%short_url%'] = str_replace("%0A", "", $vars['%short_url%']);
		// \End Edit
	...
	private function verify_twitter_credentials($username, $password){
		if(function_exists('curl_init')){
			...
			// the URL it's going to AKA the twitter status updater.
			curl_setopt($twitter['ch'], CURLOPT_URL, 'http://twitter.com/account/verify_credentials.xml');
			
			$result = curl_exec($twitter['ch']);	
			$resultArray = curl_getinfo($twitter['ch']);
			
			if ($resultArray['http_code'] == 200){
				$twitter_results = new SimpleXMLElement($result);
				if(is_object($twitter_results->user)){
					return TRUE;
				}
				return FALSE;
			} else {
	...
	public function st_install($upgrade=NULL){
		...
		if($upgrade == NULL){
			...
			add_option('st_twitter_apikey', '');
			
			// bit.ly
			add_option('st_bitly_user', '');
			add_option('st_bitly_apikey', '');
			
			add_option('st_url_shortening_service', 'isgd');
			add_option('st_enable_api', TRUE);
			add_option('st_hide_donate', TRUE);
		...
		add_option('st_current_version', '1.6.5');
	}
...
if(!function_exists('show_social_widgets') && !function_exists('show_social_icons')){ // stops it screwing with other plugins.
	function show_social_widgets($icons=NULL, $custom=NULL, $permalink=NULL, $title=NULL, $excerpt=NULL){
		global $ql;
		$ql->show_social_widgets($icons,$custom, $permalink, $title, $excerpt);
	}
	function show_social_icons($icons=NULL, $custom=NULL, $permalink=NULL, $title=NULL, $excerpt=NULL){
		global $ql;
		$ql->show_social_widgets($icons,$custom, $permalink, $title, $excerpt);
	}
}
